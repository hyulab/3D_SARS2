...
parser.add_argument('receptor', nargs='?', default=sys.stdin, help='File name of input receptor PDB file (.pdb or .ent).')
parser.add_argument('ligand', nargs='?', default=sys.stdin, help='File name of input ligand PDB file (.pdb or .ent).')
...
receptorinfile = open_pdb(args.receptor)
ligandinfile = open_pdb(args.ligand)

#------------------------------ PARSE PDB FILE ---------------------------------

pdbatomdict = {chain1:[], chain2:[]}

for line in receptorinfile:
	if line is not None and line[:4] == "ATOM":
		line = line[:21] + chain1 + line[22:]
		pdbatomdict[chain1].append(line)
receptorinfile.close()

for line in ligandinfile:
	if line is not None and line[:6] == "HETATM":
		line = (line[:21] + chain2 + line[22:]).replace("HETATM", "ATOM  ")
		pdbatomdict[chain2].append(line)
ligandinfile.close()
