...
init() # WARNING: option '-constant_seed' is for testing only! MAKE SURE TO REMOVE IT IN PRODUCTION RUNS!!!!! import os; 
...
def scanning(pdb_filename, partners, mutant_aa = 'A',
        ...
        trials = 1, trial_output = '', iresA="", iresB=""):
    """
    ...
    iresA = [pose.pdb_info().pdb2pose(partners[0], int(x)) for x in iresA.split(",")]
    iresB = [pose.pdb_info().pdb2pose(partners[-1], int(x)) for x in iresB.split(",")]
    ...
    for trial in range( trials ):
        ...
        for i in range(1, pose.total_residue() + 1):
            ...
            if i in iresA or i in iresB:
                ...
                if output:
                    ...
                    outdir = "Structures/{0}/".format("_".join(os.path.basename(filename).split("_")[:2]))
                    if(not os.path.exists(outdir)):
                        try:
                            os.mkdir(outdir)
                        except OSError:
                            pass
                    outdir = "Structures/{0}/Mut_to_{1}/".format("_".join(os.path.basename(filename).split("_")[:2]), mutant_aa)
                    if(not os.path.exists(outdir)):
                        try:
                            os.mkdir(outdir)
                        except OSError:
                    ...
                    filename = os.path.abspath("{0}{1}_{2}{3}{4}_{5}.pdb".format(outdir, "_".join(os.path.basename(filename).split("_")[:2]), pose.sequence()[i-1], pdb_pos, mutant_aa, chain, trial))
                ...
                ddG_mutants[i] = interface_ddG(pose, i, mutant_aa,
                    movable_jumps, ddG_scorefxn, interface_cutoff, filename, iresA, iresB)
        ...
        f.close()
        while(not os.path.exists(trial_output + '_' + str(trial + 1) + '.txt')):
            f = open(trial_output + '_' + str(trial + 1) + '.txt' , 'w' )
            f.writelines(display)
            f.close()
...
def interface_ddG( pose, mutant_position, mutant_aa, movable_jumps, scorefxn = '',
        cutoff = 8.0, out_filename = '', iresA="", iresB=""):
    ...
    wt_score = calc_binding_energy(wt, scorefxn,
        mutant_position, cutoff, out_filename.replace(".pdb", "WT_Complex.pdb"))
    mut_score = calc_binding_energy(mutant, scorefxn,
        mutant_position, cutoff, out_filename.replace(".pdb", "Mut_Complex.pdb"))
    ...
    #
    #
    # NOTE: THIS SHOULD BE UNCOMMENTED, BUT I'M GETTING A WRITE ERROR WHEN I TRY TO DUMP
    #
    #
    #
    #if out_filename:
...
def calc_binding_energy(pose, scorefxn, center, cutoff = 8.0, out_filename=""):
    ...
    if(out_filename):
        test_pose.dump_pdb(out_filename.replace("Complex", "Separate"))
...
parser.add_option('--iresA', dest = 'iresA',
    default = 'None',
    help = 'Set of interface residues on chain A')
parser.add_option('--iresB', dest = 'iresB',
    default = 'None',
    help = 'Set of interface residues on chain B')
...
# Move into base directory for outputs
os.chdir(options.out_base)
...
if(not os.path.exists(os.path.dirname(trial_output))):
    try:
        os.mkdir(os.path.dirname(trial_output))
    except OSError:
        pass


scanning(pdb_filename, partners, mutant_aa,
    interface_cutoff, output, trials, trial_output, iresA, iresB)
